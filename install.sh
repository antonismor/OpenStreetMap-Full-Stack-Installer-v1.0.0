...
download_country_menu() {
  while true; do
    ...
    printf "  1) Countries only (ISO-3166 list)\n"
    printf "  2) All Geofabrik extracts (countries, states, regions)\n"
    printf "  3) Download ALL country extracts (very large)\n"
    printf "  4) Download full planet PBF\n"
    printf "  5) Show downloaded datasets\n"
    printf "  0) Back\n\n"
    read -rp "Selection: " c
    case "$c" in
      1|2)
        ...
        pause
        ;;
      3) download_all_countries; pause ;;
      4) download_planet; pause ;;
      5) banner; find "$DATA_DIR" -maxdepth 1 -type f -printf '%TY-%Tm-%Td %TH:%TM  %10s  %f\n' | sort; pause ;;
      0) return ;;
      *) warn "Invalid selection." ;;
    esac
  done
...
advanced_menu() {
  while true; do
    ...
    printf "  4) Import selected PBF -> Overpass\n"
    printf "  5) Configure rendering replication timer\n"
    printf "  6) Configure HTTPS with Certbot\n"
    printf "  7) Backup configuration now\n"
    printf "  8) Restart all installed services\n"
    printf "  0) Back\n\n"
    read -rp "Selection: " c
    case "$c" in
      ...
      7) /usr/local/sbin/osm-backup-configs.sh 2>/dev/null || warn "Backup helper not installed."; pause ;;
      8)
        for s in postgresql apache2 renderd nominatim.socket nominatim.service osrm.service overpass-dispatcher.service overpass-areas.service overpass-rules.service; do
          systemctl list-unit-files | grep -q "^${s}" && run systemctl restart "$s" || true
        done
        success "Installed services restarted."; pause ;;
      0) return ;;
...
main_menu() {
  ...
  while true; do
    ...
    hardware_report
    printf "\n%bMAIN MENU%b\n\n" "$BOLD$CYAN" "$RESET"
    printf "  %b1)%b FULL SOFTWARE INSTALL — all stack components\n" "$GREEN" "$RESET"
    printf "  %b2)%b Full Auto Country Deployment — download + import everywhere\n" "$GREEN" "$RESET"
    printf "  3) Country / Region Download Manager\n"
    printf "  4) Advanced imports / updates / HTTPS / maintenance\n"
    printf "  5) Health / service status\n"
    printf "  6) Show installer log\n"
    printf "  0) Exit\n\n"
    read -rp "Selection: " choice
    case "$choice" in
      1) full_install ;;
      2) full_auto_country_deployment ;;
      3) download_country_menu ;;
      4) advanced_menu ;;
      5) health_screen ;;
      6) less +G "$LOG_FILE" ;;
      0) banner; printf "%bBye.%b\n" "$GREEN" "$RESET"; exit 0 ;;
